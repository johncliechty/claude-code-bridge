...
if ! command -v git >/dev/null 2>&1; then
    ...
    fail "Install with Homebrew: brew install git"
    fail "Or install Xcode Command Line Tools: xcode-select --install"
    exit 1
fi
...
if [[ ! -d "$BRIDGE_ROOT/.git" ]]; then
    ...
    # Optionally pull latest to keep idempotent runs in sync
