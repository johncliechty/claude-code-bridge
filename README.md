...
The previously-shipped `Install-Claude-Code-Bridge.bat` (a "right-click → Save link as → double-click" convenience installer) **has been removed from the repo.** Its internal `powershell.exe -ExecutionPolicy Bypass -Command "iex (iwr ...).Content"` invocation is the exact pattern Windows 11 Smart App Control blocks at CreateProcess, and the failure mode (`Program 'powershell.exe' failed to run: Access is denied`) is silent enough that users wouldn't know to switch to the bare-iex path. Keeping a broken alternative alongside the working one was net harmful. See [`KNOWN-ISSUES.md`](./KNOWN-ISSUES.md) §7 for the SAC failure-mode write-up.
